...
01          07AUG2015     DN  	     First initiation (source code transferred from macro %AutoTableProg
02			07AUG2015	  DN		 Macro now generates working macros instead of programmes!
...
%macro AutoTableMacro (inp_path, out_path);
...
*			end;
		...
		*** define macro variables &&set&set_counter which indicates the analysis populations for which this table is relevant.
		*** at the end of this macro the written macro is called for each anaylsis population;
		data aaa;
			set programme_data;
			num_pop_sets = count(strip(analysis_population), " ")+1;
			do set_counter=1 to num_pop_sets;
				call symput('set'||trim(left(put(set_counter,8.))),scan(strip(analysis_population),set_counter," "));
				if set_counter eq num_pop_sets then call symput('num_pop_sets', num_pop_sets);
			end;
		run;
	...
	*		else call symput ("by_variable_3", "");
		...
		*** the following code is only executed in case of the first row of the SAS-dataset (e.g. writing the header of the SAS-programme, see line 235-265); 
			if _n_ eq 1 then do;
				...
				put "SAS-version:         9.4";
				put 112*"*";
				put "Short description:   Macro for table '" title_table +(-1)"'"; 
				...
				put "*** Define the macro that generates the table;";
				put '%' "macro Generate_&programme_name (input);" ;
				...
				*** the code in the next lines until line 292 is used to decide by a decision tree which macro should be used to calculate the denominator for the calculation of
				...
				if (upcase(strip(percentages_by_var)) eq "" or compress(by_variable_1) eq "") and compress(upcase(percentages_control_item)) eq "YES" and strip(control_item_name) ne "" then do;
					...
					if compress(upcase(percentages_control_item)) eq "YES" and strip(control_item_name) ne "" and upcase(compress(percentages)) ne "ALL_OBS" then put @5 "%" "CountObs(input=" '&input' "," / @21 "name=num_obs," / @16 "no_rep=" percentages +(-1) "," /@16 "condition=" condition +(-1) ");" /;
					else if strip(control_item_name) ne "" and  compress(upcase(percentages_control_item)) eq "YES" and upcase(compress(percentages)) eq "ALL_OBS" then put @5  "%" "CountObs(input=" '&input' "," / @16 "name=num_obs," / @16 "no_rep=," /@16 "condition=" condition +(-1) ");" /;
					...
					put "*** Count the number of observations in the dataset the percentages are related to;";
					if upcase(compress(percentages)) ne "ALL_OBS"  then put @5 "%" "CountObs(input=" '&input'  "," / @16 "name=num_obs," / @16 "no_rep=" percentages +(-1) "," /@16 "condition=);" /;
					else if upcase(compress(percentages)) eq "ALL_OBS" then put @5 "%" "CountObs(input=" '&input'  "," / @16 "name=num_obs," / @16 "no_rep=," /@16 "condition=);" /;
					sample_size ='&m_num_obs';
				end;
					...
					put @5"%" "SubjectsPerClass(input=" '&input'  "," / @24 "class_variables=" &percentages_by_var  +(-1) "," ;
					if compress(upcase(percentages)) ne "ALL_OBS" then put @24 "no_rep=" percentages +(-1)",";
						else put  @24 "no_rep=,";
					if strip(upcase(percentages_control_item)) eq "YES" and strip(control_item_name) ne "" then put @24 "condition=" control_item_name " in (" option_value +(-1)"));"; 
						else if strip(upcase(percentages_control_item)) ne "YES" or strip(control_item_name) eq "" then put @24 "condition=);";
					put;
				end;
			...
			***	this code is only executed if the denominator for the calculation of percentages differs from the denominator used for the analysis of the previous item;
			if _n_ > 1 and (upcase(strip(percentages_by_var)) eq "" or compress(by_variable_1) eq "") and (strip(control_item_name) eq "" or compress(upcase(percentages_control_item)) ne "YES") 
				...
				sample_size ='&m_num_obs';
														...
														condition =strip(control_item_name) || " in (" || compress(option_value) || ")";
				put "*** Count the number of observations in the dataset the percentages are related to;";
				if compress(percentages_by_var) eq "" and upcase(compress(percentages)) ne "ALL_OBS" then put @5 "%" "CountObs(input=" '&input' "," / @16 "name=num_obs_subgroup," / @16 "no_rep=" percentages +(-1) "," /@16 "condition=" condition +(-1) ");" /;
				else if compress(percentages_by_var) eq "" and upcase(compress(percentages)) eq "ALL_OBS" then put @5 "%" "CountObs(input=" '&input' "," / @16 "name=num_obs_subgroup," / @16 "no_rep=," /@16 "condition=" condition +(-1) ");" /;
				...
				put @5 "%" "SubjectsPerClass(input=" '&input'  "," / @24 "class_variables=" &percentages_by_var  +(-1) "," ;
					...
					if strip(upcase(percentages_control_item)) eq "YES" and control_item_name ne "" then put @24 "condition=" control_item_name " in (" option_value +(-1)"));"; 
						else if strip(upcase(percentages_control_item)) ne "YES" or strip(control_item_name) eq "" then put @24 "condition=);";
				put;
			end; 
		...
		*** the code in the next lines until line 363 adds a proc sort to the SAS-programme taht is currently written if necessary;
			if by_variable_1 ne "" and _n_ eq 1 then do;
				put @5 "proc sort data=" dataset";";
				put @5 "	by " by_variable_1 by_variable_2 by_variable_3 +(-1) ";";
				put @5 "run;"; 
				put;
			end;
			else if by_variable_1 ne "" and _n_ ne 1 and dataset ne lag1(dataset) then do;
				put @5 "proc sort data=" dataset";";
				put @5 "	by " by_variable_1 +(-1) by_variable_2 by_variable_3 +(-1)";";
				put @5 "run;";
				put;
			end;
		...
		*** the code in the next lines until line 539 is used to decide based on a decision tree which macro(s) should be used to analyse the current item and to define the values
			...
			*** depending on the style chosen it might be necessary to modify the input dataset prior to and/or during the analysis of the item --> This is also decided in the next lines;
			if upcase(compress(comment)) not in ("MINOR_TERM" "MINOR_MINOR_TERM") then put @5 "*** Analysis of the item;"; 
			...
			if compress(upcase(data_type)) eq "FLOATING" then do;
				put @5 "%" "SumStatistics(var=" item_name +(-1) "," / @21 "suborder=" order +(-1)"," / @21 "name_tab='" item_description +(-1)"'," / @21 "input=" '&input' "," 
					/ @21 "output=" item_name +(-1) "," / @21 "condition=" condition +(-1) "," / @21 'num_subj=' sample_size +(-1) "," / @21 "column_name=total,"
					/ @21 "class_variables=" by_variable_1 by_variable_2 by_variable_3 +(-1) "," / @21 "level=1);";
				...
				put @5 "%" "Frequency(var=" item_name +(-1)"," / @21 "suborder=" order +(-1)"," / @21 "name_tab='" item_description +(-1)"'," / @21 "input=" '&input'  "," 
					/ @21 "output=" item_name +(-1)"," / @21 "format=" format_name +(-1)"," / @21 "condition=" condition +(-1) "," / @21 'num_subj=' sample_size +(-1) ','
					/ @21 "class_variables=" by_variable_1 by_variable_2 by_variable_3 "," / @21 "column_name=total," / @21 "level=1);";
				datasets_report = strip(datasets_report) || " m_" || strip(item_name);
				put;
			end;
				...
				put @5 "%" "Frequency(var=" item_name +(-1)"," / @21 "suborder=" order +(-1)"," / @21 "name_tab='" item_description +(-1)"*'," / @21 "input=" '&input' "," 
					/ @21 "output=" item_name +(-1)"," / @21 "format=" format_name +(-1)"," / @21 "condition=" condition +(-1)"," / @21 'num_subj=' sample_size +(-1) ','
				    / @21 "column_name=total," / @21 "class_variables=" by_variable_1 %if by_variable_2 ne %then by_variable_2; %if by_variable_3 ne %then by_variable_3; +(-1)","
					/ @21 "level=2);";
...
*					set_nr = compress(order + 0.01);
					put @5 "%" "ObsDataset (input=" '&input'  "," / @18 "no_rep=" percentages +(-1) ",";
					put @18 "output=m_" item_name +(-1) order +(-1) ",";
					put @18 "order=" order "," / @18 "var_description='" description +(-1)"',"; 
					if strip(control_item_name) ne "" then put @18 "condition=" condition +(-1)",";
					else if strip(control_item_name) eq "" then 	put @18 "condition=,";
					put @18 "num_subj=" sample_size "," / @18 "column_name=total);";
*					order = order + 0.01;
...
*				order = order - 0.01;
				put @5 "data no_dups;" / @10 "set " '&input'  ";";													
				if strip(control_item_name) ne "" then put @10 "if " control_item_name "in (" option_value +(-1) ");";
				put	@10 "help_variable='" description +(-1) "';" / "run;" /; 
				if compress(percentages) ne "" and compress(percentages) ne  "ALL_OBS" then 
					put @5 "proc sort data=no_dups nodupkey;" / @10 "by " percentages +(-1) ";" / "run;" /;  
				condition =strip(control_item_name) || " in (" || compress(option_value) || ")";
*				set_nr = compress(order + 0.01);
				put @5 "%" "FreqMedCodeAuto(input=no_dups,";
				put @23 "output=m_" item_name +(-1) order +(-1) "," / @23 "umbrella_term=help_variable,";
				put @23 "minor_term=," / @23 "minor_minor_term=,";
				if compress(percentages) eq "" then put @23 "no_rep_start=" "," / @23 "no_rep_end=,";
				else if compress(percentages) ne "" then put @23 "no_rep_start=by " percentages +(-1) "," / @23 "no_rep_end=,";
				put @23 'num_subj=' sample_size +(-1) ',';
				put @23 "column_name=total," / @23 "condition=," / @23 "class_variables=" by_variable_1 by_variable_2 by_variable_3 +(-1) ");";
				...
				condition =strip(control_item_name) || " in (" || compress(option_value) || ")";
				if compress(by_variable_1) eq  "" then do; 
					put @5 "%" "ObsDataset (input=" '&input'  "," / @18 "no_rep=" %if &var_horizontal ne "" %then var_horizontal +(-1); ",";
					if upcase(strip(style)) eq "ATC" then put @18 "output=m_" item_name +(-1) "_0,";
					put @18 "order=" order "," / @18 "var_description='" description "',"; 
					if strip(control_item_name) ne "" then put @18 "condition=" condition +(-1) ",";
					else if strip(control_item_name) eq "" then 	put @18 "condition=,";
					put @18 "num_subj=" sample_size "," / @18 "column_name=total);";
					order = order + 0.01;
			...
			if upcase(strip(style)) eq "ATC" and compress(by_variable_1) ne "" and upcase(compress(comment)) not in  ("MINOR_TERM" "MINOR_MINOR_TERM") then do;
				...
				put @10 "help_variable='" description +(-1) "';" / "run;" /; 
				put @5 "%" "FreqMedCodeAuto(input=no_dups,";
				put @23 "output=m_" item_name +(-1) "_0," / @23 "umbrella_term=help_variable,";
				put @23 "minor_term=," / @23 "minor_minor_term=,";
				if compress(percentages) eq "" or compress(upcase(percentages)) eq "ALL_OBS" then put @23 "no_rep_start=," / @23 "no_rep_end=,";
				else if compress(percentages) ne "" then put @23 "no_rep_start=by " percentages +(-1) "," / @23 "no_rep_end=,";
				put @23 'num_subj=' sample_size +(-1) ',';
				put @23 "column_name=total," / @23 "condition=," / @23 "class_variables=" by_variable_1 by_variable_2 by_variable_3 +(-1) ");";
		    ...
		    if upcase(strip(style)) in ("AE" "SOC_PT" "ATC") and compress(by_variable_1) ne "" and compress(upcase(comment)) not in  ("MINOR_TERM" "MINOR_MINOR_TERM") then do;
				put @5 "%" "FreqMedCodeAuto(input=" '&input'  ","; 
				if upcase(strip(style)) eq ("SOC_PT") then put @23 "output=m_" item_name +(-1)",";
				else if upcase(strip(style)) in ("AE" "ATC") then put @23 "output=m_" item_name +(-1) "_1,";
				put @23 "umbrella_term=" item_name +(-1)  ",";
				if 		upcase(previous_comment) eq "MINOR_TERM" and upcase(prev_previous_comment) eq "MINOR_MINOR_TERM" 
					then put @23 "minor_term=" previous_item +(-1)"," /@23 "minor_minor_term=" prev_previous_item +(-1) ",";
				else if upcase(previous_comment) eq "MINOR_TERM" and upcase(prev_previous_comment) ne "MINOR_MINOR_TERM" 
					then put @23 "minor_term=" previous_item +(-1)"," /@23 "minor_minor_term=,";
				else if upcase(previous_comment) ne "MINOR_TERM" then put @23 "minor_term=," / @23 "minor_minor_term=,";
				if compress(percentages) eq "" or compress(upcase(percentages)) eq "ALL_OBS" then put @23 "no_rep_start=," / @23 "no_rep_end=,";
				else if compress(percentages) ne "" then put @23 "no_rep_start=by " percentages +(-1) "," / @23 "no_rep_end=,";
				put @23 'num_subj=' sample_size +(-1) ',';
				put @23 "column_name=total" "," / @23 "condition=" condition +(-1) "," / @23 "class_variables=" by_variable_1 by_variable_2 by_variable_3 +(-1) ");";
				put;
				if upcase(strip(style)) eq ("AE") then do;
					put @5 "%" "FreqMedCodeAuto(input=" '&input'  "," / @23 "output=m_" item_name +(-1) "_2," / @23 "umbrella_term=" item_name +(-1)  ",";
					if 		upcase(previous_comment) eq "MINOR_TERM" and upcase(prev_previous_comment) eq "MINOR_MINOR_TERM" 
						then put @23 "minor_term=" previous_item +(-1)"," /@23 "minor_minor_term=" prev_previous_item +(-1) ",";
					else if upcase(previous_comment) eq "MINOR_TERM" and upcase(prev_previous_comment) ne "MINOR_MINOR_TERM" 
						then put @23 "minor_term=" previous_item +(-1)"," /@23 "minor_minor_term=,";
					else if upcase(previous_comment) ne "MINOR_TERM" then put @23 "minor_term=," / @23 "minor_minor_term=,";
					put @23 "no_rep_start=," / @23 "no_rep_end=,";
					put @23 'num_subj=' sample_size +(-1) ',';
					put @23 "column_name=total" "," / @23 "condition=" condition +(-1) "," / @23 
				...
				if upcase(strip(style)) eq "AE" and find(upcase(strip(percentages_by_var)), "BY_VARIABLE_1", 1) gt 0 then do;
					...
					put @5 "%" "RecalcPercentages(input=m_" item_name +(-1) "_1," / 
							  @26 "freq_data=Frequencies," / @26 "class_variables=" &percentages_by_var +(-1)"," / @26 "output=" item_name +(-1)"_1);" / ;
							  datasets_report = strip(datasets_report) || " m_" || strip(item_name);
				end;
				if upcase(style) eq "AE" then do;
					...
					put @5 "proc sort data=m_" item_name +(-1) "_1;";
					put @10 "by " by_variable_1 by_variable_2 by_variable_3 "var_description;";
					put @5 "run;"; 
					put;
					put @5 "proc sort data=m_" item_name +(-1) "_2;";
					put @10 "by " by_variable_1 by_variable_2 by_variable_3 "var_description;";
					put @5 "run;"; 
					put;
					put @5 "data m_" item_name +(-1) ";";
					put @10 "length total $18.;";
					put @10 "merge m_" item_name +(-1) "_1 (rename=(total=total_old)) "  / @16 "m_" item_name +(-1) "_2 (rename=(count=count_new) drop=total);";
					put @10 "by " by_variable_1 by_variable_2 by_variable_3 "var_description;";
					put @10 "if count_new eq . then count_new = 0;";
					put @10 "total=total_old || '" "' || put(count_new, 4.);";
					put @10 "drop total_old;";
					put @5 "run;";
					put;
				end;
				if upcase(style) eq "ATC" then do;
					if by_variable_1 ne "" then do;
						put @5 "proc sort data=m_" item_name +(-1) "_0;" / @5 "by " by_variable_1 by_variable_2 by_variable_3 +(-1)";" / "run;";
						put;
						put @5 "proc sort data=m_" item_name +(-1) "_1;" / @5 "by " by_variable_1 by_variable_2 by_variable_3 +(-1)";" / "run;" /;
						put @5 "data m_" item_name +(-1) "_0;" / @10 "merge m_" item_name +(-1) "_0 (drop=order suborder)" / @16 "m_" item_name +(-1) "_1 (drop=var_description total count);"
							/ @10 "by " by_variable_1 by_variable_2 by_variable_3 ";" / @10 "order = order - 0.001;" / "run;";
						put;
						put @5 "proc sort data=m_" item_name +(-1) "_0 nodupkey;" / @10 "by " by_variable_1 by_variable_2 by_variable_3 +(-1)";" / @5 "run;" /;
					end;
					put @5 "data m_" item_name +(-1) ";" / @10 "set m_" item_name +(-1) "_0" / @19 "m_" item_name +(-1) "_1;" / "run;" /;  	
				end;
			end;
				...
				put @5 "%" "FreqMedCodeAuto(input=" '&input'  "," / @23 "output=m_" item_name +(-1) "," / @23 "umbrella_term=" item_name +(-1)  ",";
				if 		upcase(previous_comment) eq "MINOR_TERM" and upcase(prev_previous_comment) eq "MINOR_MINOR_TERM" 
					then put @23 "minor_term=" previous_item +(-1)"," /@23 "minor_minor_term=" prev_previous_item +(-1) ",";
				else if upcase(previous_comment) eq "MINOR_TERM" and upcase(prev_previous_comment) ne "MINOR_MINOR_TERM" 
					then put @23 "minor_term=" previous_item +(-1)"," /@23 "minor_minor_term=,";
				else if upcase(previous_comment) ne "MINOR_TERM" then put @23 "minor_term=," / @23 "minor_minor_term=,";
				put @23 "no_rep_start=" "," / @23 "no_rep_end=" "," / @23 "num_subj=" sample_size +(-1) "," / @23 "column_name=total" "," / @23 "condition=" condition +(-1) "," 
				  / @23 "class_variables=" by_variable_1 by_variable_2 by_variable_3 +(-1) ");";
				put;
				datasets_report = strip(datasets_report) || " m_" || strip(item_name);
			end;
		...
		*** the code in the next lines until line 592 is used to decide based on a decision tree if it is necessary to recalculate the percentages in the dataset with the results of the analysis of the item
			...
			if compress(by_variable_1) ne "" and compress(comment) eq "" and upcase(strip(style)) not in ("AE" "ATC" "SOC_PT" "OBSDATASET") then do;
				if compress(percentages_by_var) ne "" then do;
					put "*** Recalculate the percentages. Necessary since the macro parameter 'num_subj' is equal to '.'" / 
							 @5 "and the percentages are to be calculated separately for the class-variables;";
					put @5 "%" "RecalcPercentages(input=m_" item_name +(-1) ",";	
					put @26 "freq_data=Frequencies," / @26 "class_variables=" &percentages_by_var +(-1)"," / @26	"output=" item_name +(-1)");";
				end;
				put;
				if var_horizontal ne "" then do;
					...
					put @5 "%" "ReformatFrequencies(input=m_" item_name +(-1) "," / @26 "output=" item_name +(-1) "," / @26 "var_horizontal=" by_variable_1 +(-1) "," / @26
						"var_vertical_1=var_description," / @26 "var_vertical_2=" by_variable_2 +(-1) "," / @26 
						"var_vertical_3="  by_variable_3 +(-1) "," / @26   "column_name=total);";
					put;
				end;
				put;
			end;
				...
				if compress(percentages_by_var) ne "" then do;
					put "*** Recalculate the percentages. Necessary since the macro parameter 'num_subj' is equal to '.'" /
							 @5 "and the percentages are to be calculated separately for the class-variables;";
					put @5 "%" "RecalcPercentages(input=m_" item_name +(-1) order +(-1)",";	
					put @26 "freq_data=Frequencies," / @26 "class_variables=" &percentages_by_var +(-1)"," / @26	"output=" item_name +(-1) order +(-1)");";
				put;
				end;
				if var_horizontal ne "" then do;
					put "*** Reformat the frequencies so that the results for each characteristic of the first class variables are displayed in their own column in the final table;";
					put @5 "%" "ReformatFreqMedCode(input=m_" item_name +(-1) order +(-1) "," / @26 "output=" item_name +(-1) order +(-1) "," / @26 "umbrella_term=help_variable," / @26
						"var_horizontal=" by_variable_1 +(-1) "," / @26 "var_vertical_1=var_description," / @26 "var_vertical_2=" by_variable_2 +(-1) "," / @26 
						"var_vertical_3="  by_variable_3 +(-1) "," / @26   "column_name=total);";
					put;
				end;
				put;
				...
				if compress(percentages_by_var) ne "" and upcase(strip(style)) ne "AE" then do;
					put "*** Recalculate the percentages. Necessary since the macro parameter 'num_subj' is equal to '.'" /
							 @5 "and the percentages are to be calculated separately for the class-variables;";
					put @5 "%" "RecalcPercentages(input=m_" item_name +(-1) "," / @26 "freq_data=Frequencies," / @26 "class_variables=" &percentages_by_var +(-1)"," / @26
					"output=" item_name +(-1)");" / ;
				end;
				if var_horizontal ne "" then do;
					put "*** Reformat the frequencies so that the results for each characteristic of the first class variables are displayed in their own column in the final table;";
					put @5 "%" "ReformatFreqMedCode(input=m_" item_name +(-1) "," / @26 "output=" item_name +(-1) "," / @26 "umbrella_term=" item_name +(-1) "," / @26
						"var_horizontal=" by_variable_1 +(-1) "," / @26 "var_vertical_1=var_description," / @26 "var_vertical_2=" by_variable_2 +(-1) "," / @26 
						"var_vertical_3="  by_variable_3 +(-1) "," / @26   "column_name=total);";
					put;
				end;
		...
		*** the code in the remaining lines is only executed if the end of the SAS-dataset "programme_data" is reached;
			*** the datasets with the analysis results of the different items are assembled in a single dataset, the dataset is ordered and the proc report is written;
			if eof then do;
				...
				put @5 "data data_&programme_name;"; 
				if upcase(strip(style)) not in  ("AE" "SOC_PT" "ATC") then put @10 "set " datasets_report ";";
				else if upcase(strip(style)) in ("AE" "SOC_PT" "ATC") then put @10 "set m_" item_name +(-1) ";";
				put @5 "*if compress(upcase(var_description)) ne 'OVERALL';";
				if compress(by_variable_1) ne "" and var_horizontal eq "" then put  @10 "if compress(" by_variable_1 +(-1) ") eq '.' then " by_variable_1 " = '';"
																				  / @10 "*if compress(upcase(" by_variable_1 +(-1)")) ne 'OVERALL';";
				if compress(by_variable_2) ne "" then put  @10 "if compress(" by_variable_2 +(-1) ") eq '.' then " by_variable_2 " = '';"
														 / @10 "*if compress(upcase(" by_variable_2 +(-1)")) ne 'OVERALL';";
				if compress(by_variable_3) ne "" then put  @10 "if compress(" by_variable_3 +(-1) ") eq '.' then " by_variable_3 " = '';";
				put @10 "dummy = 1;";
				put @5 "run;";
				put ;
				put @5 "proc sort data=data_&programme_name;";
				put @5 "	by order suborder;";
				put @5 "run;" /;
				if indicator_intended_entry eq 1 then do;
					...
					put @5 "%" "SplitInsertCharAuto(input=data_&programme_name," / @26 "output=data_&programme_name," / @26 "var=var_description," / @26 "width=31.27," / @26 "delimiter='^n');" /;
					put @5 "data data_&programme_name;" / @5 "set data_&programme_name;" / @5 "rename m_var_description=var_description;" / "run;" /; 
				end;
				if var_horizontal ne "" then put @5 "%" 'put &all_format_char;' /;
				put;
				put @5 'ods proclabel="Table &counti: ' title_table +(-1) '";';
				put @5 'title1 bold j=l height=10pt font=curier "^R'"''" 'Table &counti:' title_table +(-1) '";';
				put @5 'title2 j=l height=10pt font=curier "^R'"''" '&population' '";';

				put 39*"*"  " Table generation by proc report "  40*"*" ";";
				put @5 "proc report data=data_&programme_name headline headskip nowd split='|' missing contents='';";
				...
				*** the following if-statement determines the number and name of the columns of the table depending on the appearance of the table;
				if compress(by_variable_1) eq "" then put @5 "	  columns (dummy order var_description total);";	
				else if compress(var_horizontal) eq "" then put @5'   columns (dummy  ' by_variable_1 by_variable_2 by_variable_3 ' order var_description total);';
				else if compress(var_horizontal) ne "" then put @5 '	 columns (dummy ' by_variable_2 by_variable_3 ' order var_description &all_format_char);';
				put @5 "	define dummy / order noprint;"; 
				put @5 "	define order / order order=data noprint;";
				...
				***the following three if-statements determine how the columns are processed depending on the number of class-variables (by-variables) and the appearance of the table;
				if compress(by_variable_1) ne "" and var_horizontal eq "" then put @5  "	define " by_variable_1  "/ order order=data flow style(column)=[cellwidth=2.5 cm];";
				if compress(by_variable_2) ne "" then put @5  "   define " by_variable_2  "/ order order=data group flow style(column)=[cellwidth=2.5 cm];";
				if compress(by_variable_3) ne "" then put @5  "   define " by_variable_3  "/ order order=data group flow style(column)=[cellwidth=2.5 cm];";
				put @5 "	define var_description / ' ' flow style(column)=[cellwidth=6 cm];";
				if var_horizontal eq "" then put @5 '   define total  / " Frequency  |  (N = &m_num_obs)   |  n   (%)" flow style(column)=[cellwidth=4 cm] left;';
				if upcase(strip(style)) ne "OBSDATASET" and compress(by_variable_1) ne "" or compress(upcase(style)) in ("ATC" "" "SOC_PT" "AE") then do;
					put @5 "	compute before order;";
					put @5 "		line ' ';";
					put @5 "	endcomp;";
				end;
				put @5 "	compute after;";
				put @5 "		line ' ';";
				put @5 "	endcomp;";
				put @5 '	break before dummy / contents="" page;';
				put @5 '	footnote1 font=curier height=9pt';
				put @5 '	j=l "N: Number of subjects"';
				put @5 '   j=l "n: Number of subjects with data available"';
				if indicator_temp_subgroup eq 1 then put @5 '   j=l "*: Percentage based on subjects that fulfill the if-condition"';
				put @5 '	j=l "%: Percentage based on N";';
				put @5 "run;"; 
				put;
				put @5 "proc datasets library=work nolist; delete data_&programme_name; run; quit;";
				put '%' "mend Generate&programme_name (input);" ;
				put;
				...
				*** call the macro for each analysis population indicated in the Excel template;
				put 50*"*" " Macro Call " 50*"*" ";";
				put "*** execute the macro for each relevant analysis population;";
				%do set_counter=1 %to &num_pop_sets;
					put '%' "Generate_&programme_name (input=&&set&set_counter.._" dataset +(-1) ", population=&&set&set_counter"');';
				%end;
				put;
				put "ods rtf close;";
				put "ods _all_ close;";
