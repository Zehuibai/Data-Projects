---
title: 'SARS-CoV-2 vaccine acceptance in Germany'
author: "Zehui Bai"
output:
  html_document:
    df_print: paged
...
P8 <- ggplot(data = Project2 %>% filter(ImpfZust != "")  %>% filter(Household_size != "" ),
  ...
  ggtitle(title="Demographic: Gender") + xlab(".....") + ylab(".....")
  theme_set(theme_grey())+
  theme(legend.position="bottom")
...
ggarrange(P1, 
          ...
          ncol = 2, nrow = 5) %>%
  annotate_figure( top = text_grob("Histogram/Bar chart to explore socio-demographic characteristics", face = "bold", size = 12))
...
ggarrange(P10, P11, P12, P13,
          ...
          labels = c( "J", "K", "L", "M"),
          ncol = 2, nrow = 2)%>%
  annotate_figure( top = text_grob("Histogram/Bar chart to explore variables of trust in HCPs and authorities", face = "bold", size = 12))
```
